...
class Line extends LineBase {
	...
	var head:FillElement;
	var end:FillElement;

	var remove_from_buffer:Line->Void;
	var is_erased:Bool = false;

	var element(default, null):LineElement;
	var thick(get, set):Int;

	function new(point_from:Vector2, point_to:Vector2, element:LineElement, remove_from_buffer:Line->Void, head:FillElement, end:FillElement, color:Color) {
	...
	private function get_thick():Int {
		return element.thick;
	}

	var cap_offset:Float = 0.3;

	private function set_thick(value:Int):Int {
		element.thick = value;
